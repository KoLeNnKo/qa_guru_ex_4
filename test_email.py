...
email = {
    "subject": "Quarterly Report",
    "from": "kabanov@Company.com ",
    "to": " qa_guruh@Company.com ",
    "body": "Hello Bob,\n\tHere is the quarterly report.\n\tPlease review and let me know your feedback.\n\nBest,"
            "\nAlice"
}
...
corp_domains = [
    'company.ru', 'corporation.com', 'university.edu','organization.org', 'company.org', 'business.net'
]

unique_person_domains  = list(dict.fromkeys(person_domains))
unique_corp_domains = list(dict.fromkeys(corp_domains))
...
списка одновременно.
"""
if set(unique_person_domains) & set(unique_corp_domains):
    print("Дубль!")
else:
...
"""
is_corporate = domain in corp_domains
...
"""
number_of_pages = math.ceil(len(email["sent_text"]) / 500)
...
"""
personal_domains = [domain for domain in person_domains if domain not in ["list.ru", "bk.ru"]]
